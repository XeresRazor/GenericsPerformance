...
public struct Vector4Generic <T: ArithmeticFloatingPointType> {
	...
	public subscript (index: Int) -> T {
		assert(index >= 0 && index < 4, "Index must be in the range 0...3")
			...
			case 3:
				return self.w
			default:
				fatalError("Index out of bounds accessing Vector4Generic")
			}
	}
	
	public func normalize() -> Vector4Generic<T> {
	...
	public func dot(vector: Vector4Generic<T>) -> T {
...
// MARK: - Operators -
//
public prefix func - <T: ArithmeticFloatingPointType>(vector: Vector4Generic<T>) -> Vector4Generic<T> {
	return Vector4Generic(-vector.x, -vector.y, -vector.z, -vector.w)
}
